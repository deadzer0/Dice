...
class Dice {
    ...
    private function addRuleTo(Dice $dice, string $name, array $rule) {
        ...
        $dice->rules[ltrim(mb_strtolower($name), '\\')] = array_replace_recursive($dice->getRule($name), $rule);
//        $dice->rules[ltrim(strtolower($name), '\\')] = array_replace_recursive($dice->getRule($name), $rule);
    ...
    public function getRule(string $name): array {
        $lcName = mb_strtolower(ltrim($name, '\\'));
//        $lcName = strtolower(ltrim($name, '\\'));
     ...
     * @throws \ReflectionException
     */
    private function expand($param, array $share = [], bool $createFromString = false): mixed
    {
        if (is_array($param)) {
     ...
     * @return mixed
     */
    private function matchParam(\ReflectionParameter $param, $class, array &$search): mixed
    {
